  ...
  $year=$_POST['year'];
      ...
      if($category=='none' || $department == 'none' || $year == 'none')
          ...
          $results=$query->fetchAll(PDO::FETCH_OBJ);
            ...
            $sql = "SELECT user.name as project_by,user.category,user.department,title,YEAR(date_of_project) as Year,weblink,description,budget from project INNER JOIN user ON user.user_id = project.user_id where user.category=".':category'." and user.department=".':department'." and YEAR(date_of_project)=".':year';
            //Prepare the query:
            $query = $con->prepare($sql);
